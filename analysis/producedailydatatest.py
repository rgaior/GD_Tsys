...
for y in years:
    for d in range(1,constant.doy[y],1):
        ...
        if (len(daydataset.date) == 0):
            continue
#theta_20_0_phi_0_-5_expectedparam_2016_domo.txt
#exptemp_domo_-10_-20_2015_31.txt 
        oritheta = constant.GDantennageo[stname][0]
        oriphi = constant.GDantennageo[stname][1]
        if flux=='test':
