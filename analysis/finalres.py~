...
myticks = datearray
figres = plt.figure(figsize=(5,12))
figres.suptitle(stname,fontsize=15,fontweight='bold')
figres.subplots_adjust(left=0.25, bottom=0.1, right=0.94, top=0.94,
                       wspace=None, hspace=None)
#ax = plt.subplot(111)
theres = (tsys)*np.ones(len(y))
errtheres = errorwmean*np.ones(len(y))
xmin = np.min(tsysall-sigmatsysall)
xmax = np.max(tsysall+sigmatsysall)
plt.errorbar(tsysall,y, xerr=sigmatsysall,lw=4, fmt='o',alpha=0.9,label='stat. and sys.')
plt.errorbar(tsysall,y, xerr=sigmatsysstat,color='y', fmt='o',lw=4, alpha=0.9,label='stat. only')

plt.yticks(y,myticks)
plt.fill_betweenx(y, theres-errtheres, theres +  errtheres,facecolor='red',alpha=0.5)
...
fighmax = plt.figure()
plt.plot(adate,ahmax,'o')
plt.plot(adate,asimhmax,'o')
figmax = plt.figure()
plt.plot(adate,amax)
